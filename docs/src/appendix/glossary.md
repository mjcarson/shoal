...
| Admission bound | Backpressure | `networking.max_queued_queries`: the shard that accepts a bundle sheds a query bound for another shard whose mesh queue already holds that many messages, `Shedding` at once and nothing enqueued; the channel stays unbounded ([Resolved #15](resolved/shard-mesh-admission.md)) |
