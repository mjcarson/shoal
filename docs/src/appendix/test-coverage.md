...
`cargo test --workspace` passes: ~~**1,187 tests**~~ **1,198 tests**, two ignored, plus **13**
behind `--features stage-profile` that a default run does not reach.
...
| `shoal-bench` integration | 39 | committed artifacts, chart geometry, CSS sync. Up 2 with [F17](../features/workload-grid.md), both guarding the committed corpus against the four fields it added, and 2 more with [F19](../features/chart-legends.md) over the legend's layout. **22 with `--features stage-profile`**, which adds `stage_join.rs` — the only test here that starts a server ([Resolved #76](resolved/stage-join.md)) |
...
| `pool` | 13000-13002 |
| `disk_lookups` | 13000 |
| `tracing_topology` | 13000-13001 |
| `trace_propagation` | 13000-13001 |

**It is eleven binaries now, not two.** This table listed the two persistent ones; the two ephemeral
...
zero. `tracing_topology` arrived with
[Resolved #89](resolved/fragmented-query-traces.md) and binds two, 13000 and 13001, because it
restarts its server to force a partition off disk — so it is exposed the same way `disk_lookups` is
and on one more number. `trace_propagation` arrived with
[F35](../features/wire-trace-context.md) and binds the same two for the same reason, which makes
13000 and 13001 the pair three binaries now contend for. It is not exempt from that just because it
is feature gated: a `--workspace` run enables `otel` through `shoal-bench` and builds it, so it
contends exactly as the other two do.
