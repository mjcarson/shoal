...
**Items 160 to 168 and [F56](../features/cluster-rebuild.md) added 11**, 1,663 → 1,674: eight in
`cluster_fixture.rs` (`a_start_that_ends_without_a_shutdown_leaves_no_empty_archive`,
`an_unreadable_partition_stalls_one_copy_and_repairs_it`, `a_stalled_copy_survives_a_restart_and_an_operator_repairs_it`,
`a_restore_with_failed_groups_is_finished_by_a_retry`, `an_archive_compaction_leaves_a_corrupt_record_and_quarantines_it`,
`a_segment_merge_onto_a_corrupt_record_has_its_copy_repaired`, `a_stalled_leader_hands_its_lead_on_first`,
`a_copy_moved_in_after_a_restore_holds_the_restored_rows`), two in `shoal-core`
(`a_late_cut_does_not_answer_for_a_newer_scrub`, `a_restore_retry_drives_only_its_failed_groups`) and one
in `shoalctl` (`a_rebuild_needs_every_other_member_up_and_no_plan`). The workspace run at six threads,
with `--no-fail-fast` and nothing else on the host, passed 1,667 with seven ignored. An earlier run with a
release build and a lab upgrade on the same host failed `a_query_for_a_shard_that_fell_behind_is_shed`,
which passed alone three of three. The tests for #163, #164 and #167 run the paths but do not reproduce the
lab's timing, as their pages say.

**[Resolved #159](resolved/map-ahead-of-archive.md) added 2**, 1,661 → 1,663:
`a_crash_mid_compaction_leaves_every_note_readable` in `cluster_fixture.rs` and
`a_logged_entry_past_its_archive_keeps_the_one_before` in `shoal-core`'s `fs/map.rs`. The workspace
run at six threads gave 1,654 passed, 2 failed and seven ignored. The two failures,
`scheduled_scrub_quarantines_without_an_operator` and `local_rehome_recovers_after_each_crash_point`,
are on [item 142](known-issues.md#142-two-fixture-tests-fail-intermittently-on-an-idle-host)'s list
of loaded failures, and each passed alone three times out of three.

**[Resolved #158](resolved/runtime-waker-lists.md) added 5**, 1,656 → 1,661, all `shoal-core` unit tests: `an_unchanged_watch_keeps_one_waker_per_receiver` and `a_dropped_receiver_leaves_no_waker` in `runtime/watch.rs`, and `a_waiter_is_woken_past_an_abandoned_one`, `an_abandoned_wake_is_handed_on` and `a_waiter_polled_again_keeps_one_waker` in `runtime/mutex.rs`. The workspace run at six threads passed every test, with seven ignored.

**Resolved #156 (partly) added 1**, 1,655 → 1,656: `a_node_whose_wal_cannot_be_written_stops` in `cluster_fixture.rs`.

**Resolved #155 (partly) added 1**, 1,654 → 1,655: `a_restore_rides_out_an_unreachable_member` in `cluster_fixture.rs`.

**Resolved #157 added 1**, 1,653 → 1,654: `a_root_that_cannot_be_removed_is_emptied` in the `shoalctl` unit tests.

**Resolved #153 and #154 added 2**, 1,651 → 1,653: `cleaning_up_an_install_needs_no_install_directory` in the
`shoal-core` unit tests and `a_followed_records_failed_groups_are_read_out` in the `shoalctl` unit tests.
...
`a_node_file_names_its_memory_as_the_nodes_budget` in the `shoalctl` unit tests. The workspace run at six threads
after them: **1,639 passed, 1 failed, seven ignored**, the failure item 142's `lost_response_retry_returns_original_result`.
