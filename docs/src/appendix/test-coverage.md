...
`cargo test --workspace` passes: **1,155 tests**, two ignored, plus **13** behind
...
| Where | Tests | |
| --- | --- | --- |
| `shoal-proto` unit | 182 | the protocol, the SHQL parser, SCRAM, the TLS config — moved out of `shoal-core`. Up 4 with [F28](../features/rearchived-rows.md), over the mirror that writes an archived value back into its own layout and over `RowRef`'s archived variant |
| `shoal-core` unit | 158 | the engine: partitions, storage, the shard. Was 323 before the split. Up 5 with [F23](../features/self-sizing-staging-buffer.md), all of them over the staging buffer's sizing rule, and 3 with [F25](../features/read-buffers-are-filled-not-zeroed.md) over `RequestBody` — a body delivered in pieces, a stream that ends early, and an empty one; and 2 with [F28](../features/rearchived-rows.md), which are copy counts over an archived scan and its projected control — one of them replacing the F27 test that asserted the opposite |
