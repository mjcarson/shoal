...
| Obsolete configuration or removed identity | No autonomous voting/serving; follow C8/C9 replacement and orphan rules | M9: at M9a a copy a published move took from a shard is retired - its handle shut down so openraft keeps no candidate that never wins, its log forgotten, every query of its tablets refused `StaleTopology` from the map version held, its files dropped after the grace - and a source that comes back `Down` retires from its marker ([F45](../features/replica-migration.md), `retired_copy_never_serves_from_grace_files`); ~~a removed *identity* is M9b's~~ at M9b a removed identity is tombstoned and refused at observe, admit, its report and the join, and the node stops with `ShoalError::Removed` when it learns so - from its directory, from a clone of it, at any incarnation ([F46](../features/capacity-rebalancing.md), `automatic_removal_and_rejoin_preserve_fencing`) |
