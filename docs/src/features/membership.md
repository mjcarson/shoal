...
| `cluster_fixture::map_versions_install_atomically_and_resync` | Five rapid restarts behind slowed control lanes: every node on the version the cluster settles on with every member up, or past it, with three members, a client through the slowed node only moving forward and reaching it, a late client handed it or later on subscribing ([Resolved #116](../appendix/resolved/map-version-test-snapshot.md)) |
