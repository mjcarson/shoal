...
```
   crossterm EventStream            background query tasks
         ...
         handle_event          render(frame)
                    ...
                    ┌────────────┬───┴────────┬────────────────┐
                    ▼            ▼            ▼                ▼
                tab bar      query bar    error box    ratatui-hypertile panes
                          └── completion                └── TabState per pane
                                 menu                       └── content
```

The tab bar, query bar, error box, and status bar are fixed chrome laid out by `App::render`;
hypertile only tiles the content panes. The query bar is *not* one of those panes — it is sized
to its query, one row of text until the query is long enough to wrap, and hypertile splits
proportionally rather than to content. It sits directly under the tab bar so the completion menu
has room to drop down under the cursor. The error box sits under the query bar and is sized to
its message, which is **zero rows** whenever there is no error, so the layout on the ordinary
path is exactly what it would be without it.
...
The query box completes table names, projection names, field names, keywords, and boolean and null
literals. All of it comes from the client's own schema — `QuerySupport::table_names`,
`projection_names`, `table_fields`, and `table_field_validator`, all generated by `#[db]` — so
nothing is asked of the server and the suggestions cannot drift from what the query parser will
accept.
...
| --- | --- |
| Start of a query | `SELECT`, then `*` and every projection in the database, then `FROM` |
...
Queries are typed as [SHQL](../api/shql.md), so the UI inherits its limits: whole rows or one
named projection and never a column list ([F2](../features/projections.md)), a
...
A query that does not parse leaves the query in the box and puts a red `Error` box under it
holding the message. The box is only there while the error is — the first edit to the query
clears both it and the mark on the query, since an error describes the query it was parsed from
and a query that has changed is no longer that one. Moving the cursor is not an edit and does
not clear it.

Where the parser named a part of the query, that part is drawn **red and underlined** where it
was typed:

```
┌ Query ───────────────────────────────┐
│SELECT * FROM Movie WHERE bogus = 1   │
│                          ‾‾‾‾‾       │  ← bogus is red and underlined
└──────────────────────────────────────┘
┌ Error ───────────────────────────────┐
│Unknown field 'bogus'. Valid fields    │
│are: ["id", "title", "watched", "data"]│
└──────────────────────────────────────┘
```

The mark is a terminal *style* on the cells the query already occupies, not characters drawn
among it, so a query with an error on it is the same characters in the same columns as one
without. That is deliberate and load-bearing: the query box holds a string you are about to press
enter on, and a decoration made of characters is one careless edit away from becoming part of it.

Some errors are not underlined, and instead say `- at position N` in the message. That happens
when a mark would be misleading rather than merely absent: when the span covers the whole query,
which is what `Unknown table`, `Missing partition key in WHERE clause` and a failed `SELECT` all
report ([item 49](../appendix/known-issues.md#49-the-coarsest-parse-errors-report-a-span-covering-the-whole-query));
when it is empty, as `A WHERE clause is required` is; or when the text it names contains
characters that occupy no column of their own, which would put the underline somewhere other than
under the text.

Errors are checked on enter, not while you type — a query halfway through being typed is almost
always invalid, and a warning that is on by default is not a warning.

Rows from an earlier query are left in the results pane, since the query you are fixing is
usually the one that produced them, but the pane is retitled `Results (stale)` for as long as the
error is up so they stop claiming to answer what is in the box.

Server errors use the same box with no mark on the query, since there is nothing in the query to
point at.

See [item 48](../appendix/resolved/query-error-display.md) for why the display is shaped this
way.
...
`run` is bypassed by a panic — could leave the terminal in raw mode. It then recorded the error
and returned, which stopped the crash and showed the user nothing, because nothing read the field
it recorded into. Both halves are now fixed ([24](../appendix/resolved/shoalctl-panic.md),
[48](../appendix/resolved/query-error-display.md)); it records a `QueryError` that keeps the
parser's span, and the error box and the query box between them draw it:
...
The coarsest spans still cover the whole query and so cannot be underlined
([item 49](../appendix/known-issues.md#49-the-coarsest-parse-errors-report-a-span-covering-the-whole-query)),
and the query box does not scroll to bring an underlined span that wrapped off screen back into
view.

`TabState::next` and `TabState::prev` are dead code (`components/tab.rs`).
...
There are two doctests — the crate-level example (`lib.rs`), which is compile-only (`no_run`),
and `layout_query`'s, which is not. The crate's real coverage is `shoalctl/tests/completion.rs`,
34 tests that drive a tab the way the key handler does and render into a `TestBackend`, so none
of them needs a server or a real terminal ([Test Coverage](../appendix/test-coverage.md)).
