...
pub struct ClusterFacts {
    ...
    /// The repair the arm ran in the background and what the client saw across it, if it did
    ///
    /// Only the background arm carries one; absent before
    /// [F44](../../../docs/src/features/repair.md). The marks are when the repair was asked for
    /// and when its record was done, the windows are the client's distribution before, during
    /// and after it, and the counters are what every node's scrubs read - which is what a
...
/// The windows are cut at the marks rather than at what the client saw, since a scrub is not
/// an outage: the question is what it cost, and the answer is `during` read against `before`
/// ([F44](../../../docs/src/features/repair.md)).
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct BackgroundFacts {
    /// What was asked for: `verify`
    pub kind: String,
    /// When the repair was asked for, if it was inside the run
    #[serde(default, skip_serializing_if = "Option::is_none")]
