...
pub const FAMILIES: &[Family] = &[
    Family {
        ...
        what_it_measures: "One client sending a fixed mixture of reads and writes at a live server, over a table \
        ...
        how_to_read_it: "Two numbers come off every arm and they answer different questions. The **latency** is \
             ...
             sweep.",
        what_would_make_it_wrong: "Saturation. Past the knee of the throughput curve a p50 stops being a service time and \
        ...
        what_it_cannot_say: "Which half of the engine moved. A mixture blends the read path and the write path on \
    ...
    Family {
        ...
        what_it_measures: "Row width, swept sixteen ways against all four tables: thirteen fixed widths from 64 \
        ...
        how_to_read_it: "The x axis is logarithmic, so the *shape* is what to read rather than the magnitude. \
        ...
        what_would_make_it_wrong: "Sample count at the wide end. Every arm moves roughly the same number of *bytes*, so \
        ...
        what_it_cannot_say: "What a wide row costs at a *depth* other than the two measured. The mixture axis is \
    ...
    Family {
        ...
        surface: Surface::RowSize,
        what_it_measures: "The same width axis as above, on the persistent unsorted table, with **one** query \
        ...
        how_to_read_it: "Read it against the curve above, width by width. Where the two lie close together, the \
        ...
        what_would_make_it_wrong: "Reading a depth-1 throughput as a capacity. One query outstanding leaves eleven of the \
        ...
        what_it_cannot_say: "Whether the ephemeral half behaves the same way. This ladder is one table, so the \
    ...
    Family {
        ...
        what_it_measures: "The same mixture and the same rows against each of the four tables Shoal has: sorted \
        ...
        how_to_read_it: "Read this page in pairs. **Persistent against ephemeral** at the same table kind is \
        ...
        what_would_make_it_wrong: "Reading the ephemeral arms as a feature rather than as a control. Nothing an ephemeral \
        ...
        what_it_cannot_say: "What a sorted table costs when a partition holds many rows. Every arm here writes one \
    ...
    Family {
        ...
        what_it_measures: "The reference mixture with its reads drawn from three key distributions: uniform, \
        ...
        how_to_read_it: "Uniform is the honest floor: it defeats every cache in the system, so a number under it \
        ...
        what_would_make_it_wrong: "Comparing a skewed number to a uniform one and calling the difference an improvement. \
        ...
        what_it_cannot_say: "Where the knee of the throughput curve is for any arm but the reference cell. The depth \
    ...
    Family {
        ...
        surface: Surface::Access,
        what_it_measures: "The grid's reference cell at four load depths: one query outstanding, then eight, \
             thirty two - which is where every grid arm runs - and a hundred and twenty eight. \
             Everything else about the four is identical.",
        how_to_read_it: "This is a throughput-latency curve, and it is read from the shape of the two together. \
             ...
             or queue lengths.",
        what_would_make_it_wrong: "Nothing on this page is automatic. [O31](../appendix/optimizations.md) records that the \
        ...
        what_it_cannot_say: "What a second client would do. Depth and client count are separate axes - eight queries \
    ...
    Family {
        ...
        surface: Surface::TableTypes,
        what_it_measures: "Inserting rows and nothing else, saturating the pipeline so the number worth reading is \
        ...
        how_to_read_it: "This is a **per-batch** measurement: one timestamp covers a whole batch, so every query \
             ...
             case.",
        what_would_make_it_wrong: "Comparing a per-batch percentile against a per-query one from anywhere else on this \
        ...
        what_it_cannot_say: "What a write costs when the client is not saturating. A saturated pipeline is how \
    ...
    Family {
        ...
        title: "The read path",
        surface: Surface::TableTypes,
        what_it_measures: "Getting one row by its partition key, at a bounded concurrency so that a sample is a \
        ...
        how_to_read_it: "The resident arm and the archived arm are a control and its null: identical but for \
        ...
        what_would_make_it_wrong: "The archived arm **warms up as it runs**. Every partition it reads from disk stays \
        ...
        what_it_cannot_say: "What a read costs when the partition holds more than one row, which is the case a sorted \
             table exists for.",
    },
    Family {
        name: "fanout",
        ...
        what_it_measures: "One get naming *n* partition keys, swept over six values of *n* from one to two hundred \
        ...
        how_to_read_it: "This is the curve that says whether the cost of naming *n* keys is proportional to *n* \
        ...
        what_would_make_it_wrong: "Sample count, again, and warming. The widest points run the fewest queries, so their p99 \
        ...
        what_it_cannot_say: "Where the cost is. This is an end-to-end measurement over a live server, so a bend in it \
    ...
    Family {
        ...
        what_it_measures: "The client's three ways of sending - one query at a time, a batch at a time, an ordered \
             stream and an unordered one - each at a narrow row and at a MiB one, over a plaintext \
             wire and over one the kernel encrypts.",
        how_to_read_it: "**The row width is the point of this family.** At 256 bytes the four modes spread nearly \
        ...
        what_would_make_it_wrong: "Reading the ordered stream's tail as a server property. An ordered stream buffers a \
        ...
        what_it_cannot_say: "What opening a connection costs. Every arm here opens its pool before it samples \
    ...
    Family {
        ...
        surface: Surface::Transport,
        what_it_measures: "A get of one row, swept across four row widths, four load depths and four client counts, \
        ...
        how_to_read_it: "Every point is a pair, so the gap between the two halves is what encryption cost rather \
        ...
        what_would_make_it_wrong: "The depth axis leaves the useful regime at its top end. At a depth of 128 throughput \
             *falls* against depth 32, which is the signature of a queue past its knee, and four \
             ...
             not sufficient.",
        what_it_cannot_say: "What the handshake costs. The client sweep was built to see it and cannot, for the \
    ...
    Family {
        ...
        what_it_measures: "The grid's reference cell - one client, an even read/write mixture, 1 KiB rows, the \
        ...
        how_to_read_it: "Down a column, never across one. Two arms of one sweep differ in exactly one field of \
             ...
             rather than implied.",
        what_would_make_it_wrong: "Reading a flat sweep as \"this setting does not matter\" rather than as \"this setting \
        ...
        what_it_cannot_say: "What two settings are worth together. One knob moves at a time against a fixed \
    ...
    Family {
        ...
        surface: Surface::Configuration,
        what_it_measures: "The same reference cell, swept across the shard count, the per-shard memory limit, and \
             ...
             *and* at a pure read share, because they are the two settings whose effect plausibly \
             ...
             reach disk.",
        how_to_read_it: "The shard sweep is a scaling curve: read where it stops rising, not what it reaches. \
        ...
        what_would_make_it_wrong: "The client and the server share this machine. A one shard arm leaves eleven cores idle \
        ...
        what_it_cannot_say: "How Shoal scales past this machine. Every arm here runs on one host with a fixed core \
    ...
    Family {
        ...
        what_it_measures: "The grid's reference cell served by a server started with a `cluster:` block - a \
        ...
        how_to_read_it: "Beside its twin, and only there. A number here means nothing on its own; the question \
        ...
        what_would_make_it_wrong: "A control core that took a shard's core. On a machine where `resources.cores` is the \
        ...
        what_it_cannot_say: "Anything about a cluster. One node replicates to nobody, acknowledges its own writes, \
    ...
    Family {
        ...
        surface: Surface::AllWorkloads,
        what_it_measures: "One read - a get of one 1024 byte row from the ephemeral unsorted table, one \
        ...
        how_to_read_it: "As three latency distributions of the same query, read against each other from one \
        ...
        what_would_make_it_wrong: "Taking `local_shard`'s median for a pure mesh hop. The kernel picks which shard a \
        ...
        what_it_cannot_say: "What a hop costs over a network. Both nodes are processes on one machine talking over \
    ...
    Family {
        ...
        surface: Surface::AllWorkloads,
        what_it_measures: "The grid's reference mixture - half reads, the reference width, the reference depth \
        ...
        how_to_read_it: "As three distributions of the same mixture from one capture, read against each \
        ...
        what_would_make_it_wrong: "A run whose replicas ended behind. `lag_end` above a handful of entries on any \
        ...
        what_it_cannot_say: "What replication costs over a network: three processes on loopback have no \
    ...
    Family {
        ...
        surface: Surface::AllWorkloads,
        what_it_measures: "Two sets of read-only arms. `reads/one`, `reads/barrier` and `reads/session` are one \
        ...
        how_to_read_it: "`barrier` less `one` is the barrier: the leader's heartbeat round, the hop two times \
        ...
        what_would_make_it_wrong: "A run whose replicas ended behind, which turns the apply wait into follower lag; \
        ...
        what_it_cannot_say: "What a barrier costs over a network: three processes on loopback have no bandwidth, \
    ...
    Family {
        ...
        surface: Surface::AllWorkloads,
        what_it_measures: "One arm, `failover/kill`: the durable replication arm's placement and mixture - three \
        ...
        how_to_read_it: "`outage_ms` is the number: from the client's first failure to the first operation \
        ...
        what_would_make_it_wrong: "A `first_failure_ms` before `at_ms`, which means the run was failing before the fault \
        ...
        what_it_cannot_say: "What a client with a retry sees: this client has none, on purpose, so the outage is \
    ...
    Family {
        ...
        surface: Surface::AllWorkloads,
        what_it_measures: "Two arms, `catchup/log` and `catchup/snapshot`: the kill arm's placement, mixture, \
        ...
        how_to_read_it: "`seconds_to_converge` against `by` is the number: what the log costs a returning node \
        ...
        what_would_make_it_wrong: "A snapshot arm whose `by` is `log`: the node was inside the retained window after \
        ...
        what_it_cannot_say: "What a catch-up costs on a node that led nothing, or one that hosts a table other \
    ...
    Family {
        ...
        surface: Surface::AllWorkloads,
        what_it_measures: "Two arms on the kill arm's placement, mixture and client, driven for the kill arm's \
        ...
        how_to_read_it: "`during` against `before` is the number on both arms: what a scrub or a backup of \
        ...
        what_would_make_it_wrong: "A `finished_ms` that is absent, which means the run ended inside the operation and \
        ...
        what_it_cannot_say: "What a repair that installs costs, which is the snapshot arm's transfer on top of \
    ...
    Family {
        ...
        surface: Surface::AllWorkloads,
        what_it_measures: "One arm, `migration/move`: the kill arm's placement, mixture and client with a \
             ...
             second series.",
        how_to_read_it: "`during` against `before` is the number: what a move of one set costs the \
        ...
        what_would_make_it_wrong: "A `finished_ms` that is absent, which means the run ended inside the move and the \
        ...
        what_it_cannot_say: "What a move costs over a network, where the bulk lane's rate is the wire's. What \
    ...
    Family {
        ...
        surface: Surface::AllWorkloads,
        what_it_measures: "Four arms on the kill arm's placement, mixture and client, each with a plan the \
             ...
             in thousandths.",
        how_to_read_it: "`p99_ratio_permille` is the number: M9b's exit criterion is a healthy add or drain at \
             ...
             record's.",
        what_would_make_it_wrong: "An `outcome` other than `completed` on the three arms with a spare, or other than \
        ...
        what_it_cannot_say: "What a drain costs over a network, where the bulk lane's rate is the wire's and the \
    ...
    Family {
        ...
        surface: Surface::AllWorkloads,
        what_it_measures: "One arm, the one node cluster arm seeded at twelve executors and measured at eight \
        ...
        how_to_read_it: "`millis` is the number: what an operator pays in start time for changing a node's core \
             ...
             for its record.",
        what_would_make_it_wrong: "A `from` and `to` other than twelve and eight, which means the arm's counts moved and \
        ...
        what_it_cannot_say: "What a growth costs, which leaves a donor's archives holding dead records until its own \
    ...
    Family {
        ...
        surface: Surface::AllWorkloads,
        what_it_measures: "Nothing that can be reproduced. `macro/tmdb` is the pre-[F8] workload: one blended \
             insert-and-get run over a 99,999 row CSV that was never in this repository and that no \
             script fetched.",
        how_to_read_it: "As history. Seven committed captures carry it and are never rewritten, so it appears in \
             the tables and joins with nothing taken since.",
        what_would_make_it_wrong: "Any comparison at all. The dataset is gone, so a number shaped like this one would look \
             comparable and would not be - which is why no replacement was built.",
        what_it_cannot_say: "Which path moved. It was one blended number, and that is the whole reason it was \
             replaced.",
    },
];
