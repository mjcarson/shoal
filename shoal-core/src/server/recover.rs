...
pub fn force_recover(conf: &Conf, survivors: &[NodeId]) -> Result<RecoveryReport, ServerError> {
    ...
    let marker = StorageMeta::read(&root)?.ok_or_else(|| {
        ServerError::Shoal(ShoalError::InvalidConfig(format!("{} holds no storage marker to recover", root.display())))
    })?;
    ...
    let segment_bytes = conf.cluster.as_ref().map_or(10 * 1024 * 1024, |cluster| cluster.replication.segment_bytes);
    let executors = marker.physical.unwrap_or(marker.shards);
    let executor = LocalExecutorBuilder::new(Placement::Unbound).name("shoal-recover").make()?;
    let report = executor.run(async move { recover_steps(&root, me, segment_bytes, executors).await })?;
    event!(
        Level::WARN,
...
async fn recover_steps(root: &Path, me: NodeId, segment_bytes: u64, executors: usize) -> Result<RecoveryReport, ServerError> {
    // the control store, and what it has committed
    let (mut log, machine) = store::open(&root.join(CONTROL_DIR)).await.map_err(ServerError::IO)?;
    ...
    let membership = Membership::new(vec![BTreeSet::from([me])], nodes)
        .map_err(|error| ServerError::Shoal(ShoalError::InvalidConfig(format!("a membership of one: {error}"))))?;
    ...
    for executor in 0..executors {
        ...
        let wal = ShardWal::open(&dir, segment_bytes, 1 << 20).await.map_err(ServerError::IO)?;
        ...
        for group in groups {
            ...
            let entries = store.try_get_log_entries(0..).await.map_err(ServerError::IO)?;
            let last_membership = entries
                ...
                .or_else(|| checkpoint.get(group).map(|point| point.membership().membership().clone()));
            let Some(current) = last_membership else {
                continue;
            };
            // this node's address in the group, from the membership itself
            let Some(mine) = current.nodes().map(|(addr, _)| *addr).find(|addr| addr.node == me) else {
                continue;
            };
            ...
            let last_index = last.as_ref().map_or_else(
                || checkpoint.get(group).and_then(|point| point.applied.as_ref()).map_or(0, |applied| applied.index),
                |log_id| log_id.index,
            );
            ...
            let alone = Membership::<ShardAddr, ShardAddr>::new(vec![BTreeSet::from([mine])], BTreeMap::from([(mine, mine)]))
                .map_err(|error| ServerError::Shoal(ShoalError::InvalidConfig(format!("a group membership of one: {error}"))))?;
            let entry = openraft::type_config::alias::EntryOf::<DataConfig>::new_membership(log_id.clone(), alone);
            store
                .blocking_append(vec![entry])
                .await
                .map_err(|error| ServerError::GlommioGeneric(format!("appending group {group}'s recovery: {error}")))?;
            store
                .save_vote(&crate::server::wal::Vote::from_leader_id(leader, true))
                .await
                .map_err(ServerError::IO)?;
            store.save_committed(Some(log_id)).await.map_err(ServerError::IO)?;
